...
def flip_prize(product, Flag):
    url = 'https://www.flipkart.com/search?q=' + product + \
        '&otracker=AS_Query_HistoryAutoSuggest_2_0&otracker1=AS_Query_HistoryAutoSuggest_2_0&marketplace=FLIPKART&as-show=on&as=off&as-pos=2&as-type=HISTORY'
    driver.get(url)
        ...
        if Flag == False:
            for box in main_box:
                ...
                if product.lower() in val.lower():
                    price = box.find(
                        "div", {"class": "_1vC4OE _2rQ-NK"}).text.strip()
...
def amaz_price(product, Flag):
    url = "https://www.amazon.in/s?k="+product + \
        "&crid=LU6AG6TQWH25&sprefix=best+phones+%2Caps%2C314&ref=nb_sb_ss_i_1_12"
    ...
    if Flag == False:
        for box in main_box:
            ...
            link = box.find(
                "a", {"class": "a-link-normal a-text-normal"}, href=True)
            l = s+link['href']
            val = box.find(
...
def amaz_app_price(product):
    url = "https://www.amazon.in/s?k=" + product + \
        "&crid=3GQP78C68F4YO&sprefix=t%2Caps%2C395&ref=nb_sb_ss_organic-diversity_1_1"
    response = requests.get(url, headers=headers_amaz)
    soup = BeautifulSoup(response.text, 'html.parser')
